...
### Schema 校验
- `id` 与文件名一致（不含 `.md`）
- `meta_type` 在五种之一：`observation`, `insight`, `decision`, `question`, `comparison`
...
- **薄节点合并**：两个 seed/growing 节点覆盖高度相似的主题、各自正文不足 5 行 → 建议合并为一个更完整的节点
- **meta_type 修正**：标为 `insight` 但内容只是单个项目的特定决策（不可迁移）→ 建议改为 `decision`
