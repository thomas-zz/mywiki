...
### v0.3.1 — 详情页 Markdown 渲染优化 ← 计划中
- [ ] 节点详情页 body 正文 Markdown 正确渲染（标题、代码块、表格、列表、加粗/斜体等）
- [ ] 行内 `[[node-id]]` wikilink 渲染为可点击链接（带 HoverPopover）
- [ ] 代码块语法高亮（remark-prism 或 rehype-highlight）
- [ ] 表格样式适配（边框、斑马纹、响应式横向滚动）
- [ ] FolderPicker 离线模式同步支持（浏览器端 Markdown → HTML 渲染）
