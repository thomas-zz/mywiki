...
| 层 | 选型 | 理由 |
|---|---|---|
| 前端框架 | **Next.js 16 (App Router, 动态渲染)** | React 生态丰富、图组件库多、Vercel 部署零配置 |
| 样式 | **Tailwind CSS** | 快速原型、一致性好 |
| 图谱渲染 | **Cytoscape.js** | 成熟、性能好、布局算法丰富（concentric/cose/cola） |
| Markdown 解析 | **gray-matter** (frontmatter) + **remark/rehype** (正文, server) + **marked** (browser) | 线上线下统一渲染效果 |
| 语法高亮 | **rehype-highlight** (server) + **highlight.js** (browser) | 代码块着色 |
...
**数据源统一**：所有页面组件通过 `useWikiData()` hook 获取数据，hook 自动选择 `overrideData`（本地文件夹）或 `serverData`（服务端解析）。

**Server/Client 分离**：
- `app/*/page.tsx`：薄壳 server component，只传 URL 参数给 client component
- `components/views/*.tsx`：client component，承载全部 UI 逻辑
- `layout.tsx`：server component，调用 `buildWikiData()` 注入 `WikiDataProvider`

**导航统一**：线上线下统一使用 Next.js `<Link>` + `router.push`，不再有 hash routing。

### 6.2 页面组件树

```
RootLayout (WikiDataProvider → HoverPopoverProvider → Sidebar + main)
├── HomeView (views/HomeView.tsx)
│   ├── MetaTypeLegend
│   └── HomeCard[] → NodeCard (with HoverPopover)
├── NodeDetailView (views/NodeDetailView.tsx)
│   ├── BackBar
...
├── GraphView (views/GraphView.tsx)
│   └── GlobalGraph (Cytoscape cose + legend)
├── EmergenceView (views/EmergenceView.tsx)
│   └── HomeCard[] (hubs, crossDomain, questions, overload, recent, orphans)
├── DomainView (views/DomainView.tsx)
│   └── NodeList with NodeCard[]
├── AllNodesView (views/AllNodesView.tsx)
│   └── MetaTypeSection[] / StatusSection[]
└── SearchBox
...
// 弹层定位逻辑：
// 1. 跟随 trigger 元素位置
// 2. 优先左右弹出（不遮挡列表上下选择），空间足够时选右侧或左侧
// 3. 左右均不够 360px 时回退到上方弹出（使用 CSS bottom 定位，避免 translateY 闪烁）
...
### v0.3 — 体验优化 + 架构统一 ✅ 已完成
- [x] 本地目录选择（FolderPicker 组件，浏览器端解析 markdown + 构建图谱）
- [x] HoverPopover 悬浮预览组件（左右优先定位，回退上方时用 bottom 定位避免闪烁）
- [x] FolderPicker 多行 YAML 对象解析修复（relations 等字段正确解析）
- [x] 局部图谱 BFS 全连通分量展示
- [x] 详情页 Markdown 正确渲染（rehype-highlight 语法高亮 + marked 浏览器端渲染）
- [x] Wikilink `[[id]]` 渲染为可点击链接（带 data-nodeid，支持 HoverPopover）
- [x] 线上线下统一渲染架构（删除 ClientOverrideView，共享 views/ 组件）
- [x] 去掉 `output: export`，改为动态渲染（支持任意数据源的路由访问）
- [x] 导航统一 Next.js router（去掉 hash routing）

### v0.4 — 体验优化 + 图谱统一 ✅ 已完成
- [x] 搜索优化（全量子串匹配、关键词高亮、正文片段展示、结果计数）
- [x] 图谱移动端适配（缩小容器、tap 跳转、禁用 hover）
- [x] 统一图谱组件（LocalGraph + GlobalGraph → WikiGraph，共享样式）
- [x] 局部图谱 BFS 深度限制为 3 条边
- [x] 时间线视图页面（`/timeline`）：按 updated/created 垂直排列节点
- [x] validate-schema.ts 脚本（CI 校验节点 schema 完整性）
- [x] Vercel 部署（push 到 main 自动构建，删除 GitHub Actions）
- [x] GitHub 仓库数据源选择器（输入公开仓库 URL 实时加载预览）
- [x] 客户端解析逻辑抽取为 `clientParser.ts`（FolderPicker → DataSourcePicker）
