...
│
├── scripts/                     # 构建/运维脚本
│   ├── build-data.ts            # 解析 content/nodes/ → .next 构建数据
...
meta_type: model                        # observation | model | decision | question | comparison
domains:                                # 领域标签数组，至少一个
  - "#技术/网络"
...
interface WikiNode {
  id: string
  title: string
  meta_type: 'observation' | 'model' | 'decision' | 'question' | 'comparison'
  domains: string[]
  ...
  build-deploy:
    ...
    steps:
      ...
      - name: Build site (SSG)
        ...
        # Next.js static export: reads content/nodes/ at build time
...
2. 复制素材到 `content/raw/`，命名 `YYYY-MM-DD-<标题>.md`
3. 读取素材，提取知识点
4. 判断：是新建节点还是更新已有节点
5. 写/更新 `content/nodes/<id>.md`（按 schema）
6. 更新 `content/meta/index.md`
7. 追加 `content/meta/log.md`
8. 输出汇总报告
...
离线模式：本地 `pnpm dev` → Next.js dev server 实时读 content/ → 浏览器 localhost
```
...
用户可以修改 `contentDir` 指向任意本地目录。这意味着：
...
# contentDir: '/Users/zzy/my-knowledge-base/content'

# 启动本地 dev server
pnpm dev
# → http://localhost:3000 即可浏览全部 wiki 内容
# → 修改 content/ 下的 markdown 后，HMR 自动刷新
```
...
**拓扑 B：分仓（灵活）**
```
知识库仓库：my-knowledge-base/ (content/ + CLAUDE.md + raw/)
...
### v0.3 — 搜索 + 体验优化 ← 当前
- [ ] 本地目录选择（用户可指定 contentDir，dev 时实时读取）
