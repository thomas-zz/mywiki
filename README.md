...
| 命令 | 作用 |
|------|------|
| `npx mywiki-cli init [--path <dir>] [--with-samples]` | 创建 wiki 目录 + 安装 skill |
| `npx mywiki-cli panel [--port 9888]` | 启动可视化面板 |
| `npx mywiki-cli update` | 更新 skill 到最新版本 |
...
只需确保 `npx mywiki-cli init` 已完成（skill 已安装），之后直接触发 claude session 即可。
